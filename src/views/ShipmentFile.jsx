...
import { ArrowLeft, Truck, Plane, Ship, Sparkles, ScrollText, Building2, FileText } from "lucide-react";
...
export default function ShipmentFile({ onOpenAssistant }) {
  ...
  // follow the file forward as its state advances
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between" style={{ gap: 16, marginBottom: 16, flexWrap: "wrap" }}>
        <div>
          <div className="flex items-center" style={{ gap: 11 }}>
            <span style={{ fontSize: 22, fontWeight: 700, letterSpacing: "-0.01em" }}>{shipment.no}</span>
      ...
      <Panel style={{ padding: "13px 17px", marginBottom: 16 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 326px", gap: 18, alignItems: "start" }}>
        <div style={{ minWidth: 0 }}>
          {/* actions */}
          <Panel style={{ padding: "14px 17px", marginBottom: 16 }}>
            <MonoLabel style={{ display: "block", marginBottom: 11 }}>Next steps · {stateOf(shipment.state).label}</MonoLabel>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16, position: "sticky", top: 70 }}>
          <Panel style={{ padding: "15px 17px" }}>
            ...
            <div style={{ marginTop: 13, paddingTop: 12, borderTop: `1px solid ${C.lineSoft}`, display: "grid", gap: 9 }}>
              <Meta k="Assigned" v={importer ? (shipment.assigned ? (shipment.assigned === "u_broker" ? "D. Achterberg" : shipment.assigned) : "Unassigned") : "—"} vColor={shipment.assigned ? C.ink : C.warn} />
